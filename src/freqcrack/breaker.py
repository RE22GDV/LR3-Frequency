...
Простір ключів шифру складається з 26 варіантів, тому «злам» — це не
пошук, а ЗАДАЧА ВИБОРУ: перебрати всі 26 розшифрувань і вирішити, яке
з них є осмисленим англійським текстом. Уся складність зосереджена в
правилі рішення, і саме різні правила порівнює ця робота:

    ``chi2``      — критерій хі-квадрат до таблиці частот з умови задачі;
    ``unigram``   — логправдоподібність за частотами окремих літер;
    ``quadgram``  — логправдоподібність за квадриграмами;
