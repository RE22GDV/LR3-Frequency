...
> Далі збережено термінологію умови.
...
**Головний наслідок такого простору ключів:** злам — це не пошук, а
**задача вибору**. Перебрати всі 26 варіантів тривіально; уся складність
у тому, щоб автоматично вирішити, який із них є осмисленим текстом.
...
(U+2019) та інших не-літерних символів.
...
- **не-літери недоторкані** — цифри, розділові знаки, `’`, кирилиця, табуляція;
- **регістр зберігається** — посимвольна перевірка;
- **IoC інваріантний щодо зсуву**, а хі-квадрат — ні (ключова пара тестів, §6.3);
- **наївне правило помиляється систематично** — доведено формулою похибки.
...
фіксованими зернами генераторів.
...
кандидат у **37 разів** кращий за другого, а весь перебір триває **0.9 мс**.
...
|---|---|---|
| Індекс відповідності | **не змінюється взагалі** (розкид 0.0 по всіх 26 зсувах) | *тип* шифру: моноалфавітний чи ні |
...
тільки від їхнього набору, не від порядку. Тому IoC каже «це моноалфавітний
шифр», але мовчить про ключ; хі-квадрат порівнює гістограму з **конкретним**
еталоном, і тому ключ знаходить.
...
> **Наслідок для умови задачі.** CodinGame гарантує `L ≥ 67`, що з великим
> запасом перекриває поріг усіх трьох статистичних правил. Саме тому
> надісланий розв'язок із простим хі-квадратом проходить валідатори.
...
**систематично, а не випадково**: його похибка точно дорівнює зсуву
найчастішої літери *відкритого* тексту відносно E.
...
Точність правила — **57.4 %**, що добре узгоджується з часткою випадків,
коли найчастішою справді була E. Висновок методичний: правило спирається
рівно на одне спостереження, тоді як хі-квадрат використовує всі 26.
...
| Випадковий зсув на кожну позицію | 1 743 | 4 718 | **2.7×** | 0.0392 |
...
Це, звісно, не робить такий шифр придатним на практиці: ключ тут завдовжки
з повідомлення, і його потрібно передати одержувачу. Демонстрація показує
лише те, на чому саме тримається частотний аналіз.
...
2. **Злам шифру Цезаря — задача вибору, а не пошуку.** Простір ключів
   містить 26 варіантів (ентропія 4.70 біта), тому перебір тривіальний;
   уся складність — у правилі, яке автоматично впізнає осмислений текст.
...
3. **Зсув зберігає форму частотного розподілу**, лише обертаючи гістограму
   ...
   хі-квадрат, а IoC здатен лише вказати на тип шифру.
...
5. **Наївне правило помиляється систематично**: його похибка дорівнює
   зсуву найчастішої літери відкритого тексту відносно E, тому його
   точність (57.4 % на 40 символах) майже збігається з часткою текстів,
   у яких найчастішою літерою справді є E.

6. **Вимога умови про відсутність статистичних закономірностей
   підтверджується вимірюванням.** Для шифру Цезаря контраст між
   найкращим і найгіршим кандидатом становив 744 рази, для шифру з
   випадковим зсувом на кожну позицію — лише 2.7 раза, і атака перестала
   розрізняти кандидатів.

7. **Практичний урок:** стійкість шифру визначає не розмір алфавіту чи
   «заплутаність» перетворення, а те, чи лишає шифротекст статистичний
   слід відкритого тексту.
...
8. Вихідний код роботи [Електронний ресурс]. – Режим доступу:
